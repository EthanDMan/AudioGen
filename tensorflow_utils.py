...
def dense(x, intermediate, dim, name='dense',stddev=0.2,bias=True,identity=False):
    with tf.variable_scope(name):
        if identity:
            w = tf.get_variable('w', [intermediate, dim],
        ...
        dense = tf.matmul(x,w)
        if bias:
            biases = tf.get_variable('biases', [dim], initializer=tf.constant_initializer(0.0))
...
def conv2d(x, output_dim, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.2, padding='SAME', name='conv2d', is_print=True, dilate=1):
    with tf.variable_scope(name):
        #w_initial = np.ones((output_dim, x.get_shape()[-1], k_h, k_w)) 
        w = tf.get_variable('w', [k_h, k_w, x.get_shape()[-1], output_dim],
                                  #initializer=tf.constant_initializer(w_initial.transpose((3,2,1,0))))
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        if dilate > 1:    
            conv = tf.nn.conv2d(x, w, strides=[1, d_h, d_w, 1], dilations=[1, 1, dilate, 1], padding=padding, data_format='NHWC')
        else:
            conv = tf.nn.conv2d(x, w, strides=[1, d_h, d_w, 1], padding=padding, data_format='NHWC')
...
def convf2d(x, output_dim, k_h=1, k_w=256, d_h=1, d_w=1, stddev=0.2, padding='SAME', name='conv2d', is_print=True, dilate=1):
    with tf.variable_scope(name):
       ...
       # conv = tf.reshape(tf.nn.bias_add(conv, biases), conv.get_shape())
        conv = tf.nn.bias_add(conv, biases)
        conv = tf.transpose(conv, perm=[0, 3, 2, 1])
        if is_print:
            print_activations(conv)

        return conv
    
def gate_causal_conv(x,output_dim,h=None, k_h=1, k_w=1, d_h=1, d_w=1, stddev=0.5, name='causal_conv', is_print=True, dilate=1):
    with tf.variable_scope(name):
        #w_initial = np.ones((output_dim, x.get_shape()[-1], k_h, k_w)) 
        padding = tf.constant([[0,0],[k_h-1,0],[k_w-1,0],[0,0]])
        x_pad = tf.pad(x,padding,"CONSTANT")
        w = tf.get_variable('w', [k_h, k_w, x.get_shape()[-1], output_dim],
                                  #initializer=tf.constant_initializer(w_initial.transpose((3,2,1,0))))
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        conv = tf.nn.conv2d(x_pad, w, strides=[1, d_h, d_w, 1], padding='VALID')
       ...
       # conv = tf.reshape(tf.nn.bias_add(conv, biases), conv.get_shape())
        ...
        if h != None:
            v = sigmoid(tf.expand_dims(tf.expand_dims(dense(h,h.shape[1],x.shape[1]),2),3))
            conv = conv*v
...
def gated_causal_conv(x,output_dim,h=None, k_h=1, k_w=1, d_h=1, d_w=1, stddev=0.5, name='causal_conv', is_print=True, dilate=1):
    with tf.variable_scope(name):
        #w_initial = np.ones((output_dim, x.get_shape()[-1], k_h, k_w)) 
        padding = tf.constant([[0,0],[k_h-1,0],[k_w-1,0],[0,0]])
        x_pad = tf.pad(x,padding,"CONSTANT")
        w = tf.get_variable('w', [k_h, k_w, x.get_shape()[-1], output_dim],
                            ...
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        if h != None:
            v = sigmoid(dense(h,h.shape[1],k_h*k_w*x.get_shape()[-1]*output_dim,name='v'))
            v = tf.reshape(v, w.get_shape())
            w = w+v
        conv = tf.nn.conv2d(x_pad, w, strides=[1, d_h, d_w, 1], padding='VALID')
...
def causal_conv(x,output_dim, k_h=3, k_w=32, d_h=2, d_w=2, stddev=0.5, name='causal_conv', is_print=True, dilate=1):
    with tf.variable_scope(name):
        #w_initial = np.ones((output_dim, x.get_shape()[-1], k_h, k_w)) 
        padding = tf.constant([[0,0],[k_h-1,0],[k_w-1,0],[0,0]])
        x_pad = tf.pad(x,padding,"CONSTANT")
...
def causal_conv2(x,output_dim, k_h=3, k_w=32, d_h=2, d_w=2, stddev=0.5, name='causal_conv', is_print=True, dilate=1):
    with tf.variable_scope(name):
        #w_initial = np.ones((output_dim, x.get_shape()[-1], k_h, k_w)) 
        padding = tf.constant([[0,0],[k_h-1,0],[k_w-1,0],[0,0]])
        x_pad = tf.pad(x,padding,"CONSTANT")
        n = output_dim/2
        w = tf.get_variable('w', [k_h, k_w, x.get_shape()[-1], n],
                                  #initializer=tf.constant_initializer(w_initial.transpose((3,2,1,0))))
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        conv1 = tf.nn.conv2d(x_pad, w, strides=[1, d_h, d_w, 1], padding='VALID')
        
        x_pad_shift = tf.manip.roll(x_pad, shift=1, axis=1)
        conv2 = tf.nn.conv2d(x_pad_shift, w, strides=[1, d_h, d_w, 1], padding='VALID')
...
def gated_causal_conv2(x,output_dim, h=None, k_h=3, k_w=32, d_h=2, d_w=2, stddev=0.5, name='causal_conv', is_print=True, dilate=1):
    with tf.variable_scope(name):
        #w_initial = np.ones((output_dim, x.get_shape()[-1], k_h, k_w)) 
        padding = tf.constant([[0,0],[k_h-1,0],[k_w-1,0],[0,0]])
        x_pad = tf.pad(x,padding,"CONSTANT")
        ...
        w = tf.get_variable('w', [k_h, k_w, x.get_shape()[-1], n],
                            ...
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        if h != None:
            v = sigmoid(dense(h,h.shape[1],k_h*k_w*x.get_shape()[-1]*n,name='v'))
            v = tf.reduce_sum(v,0)
            v = tf.reshape(v, [k_h, k_w, x.get_shape()[-1], int(n)])
            w = w*v
...
def causal_dilated_conv2(x,output_dim, k_h=3, k_w=32, d_h=2, d_w=2, stddev=0.5, name='causal_conv', is_print=True, dilate=1):
    with tf.variable_scope(name):
        #w_initial = np.ones((output_dim, x.get_shape()[-1], k_h, k_w)) 
        padding = tf.constant([[0,0],[k_h-1,0],[k_w-1,0],[0,0]])
        x_pad = tf.pad(x,padding,"CONSTANT")
...
def deconv2d(x, k, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.2, padding_='SAME', output_size=None,
...
def causal_deconv(x, k, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.2, output_size=None,
             name='deconv2d', with_w=False, is_print=True):
    with tf.variable_scope(name):
        input_shape = x.get_shape().as_list()
        
        #padding = tf.constant([[0,0],[0,k_h-1],[0,k_w-1],[0,0]])
        # calculate output size
        h_output, w_output = None, None
        if not output_size:
            h_output, w_output = input_shape[1] * 2+k_h-1, input_shape[2] * 2 + k_w-1
        # output_shape = [input_shape[0], h_output, w_output, k]  # error when not define batch_size
        output_shape = [tf.shape(x)[0], h_output, w_output, k]
...
def causal_deconv2(x, k, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.2, output_size=None,
             name='deconv2d', with_w=False, is_print=True):
    with tf.variable_scope(name):
        input_shape = x.get_shape().as_list()
        k = int(k/2)
        ...
        w = tf.get_variable('w', [k_h, k_w, k, input_shape[3]],
                            initializer=tf.random_normal_initializer(stddev=stddev))
        w = tf.reverse(w,[0])
        deconv1 = tf.nn.conv2d_transpose(x, w, output_shape=output_shape, strides=[1, d_h, d_w, 1],
                                        padding='VALID')
        x_roll = tf.manip.roll(x, shift=1, axis=1)
        deconv2 = tf.nn.conv2d_transpose(x_roll, w, output_shape=output_shape, strides=[1, d_h, d_w, 1],
                                        padding='VALID')
        
        deconv = tf.concat([deconv1, deconv2], axis=3)
        deconv = deconv[:,k_h-1:,k_w-1:,:]
        biases = tf.get_variable('biases', [output_shape[-1]*2], initializer=tf.constant_initializer(0.0))
        deconv = tf.nn.bias_add(deconv, biases)
        if is_print:
            print_activations(deconv)
...
def gated_causal_deconv2(x, k, h=None, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.2, output_size=None,
             name='deconv2d', with_w=False, is_print=True):
    with tf.variable_scope(name):
        input_shape = x.get_shape().as_list()
        k = int(k/2)
        ...
        w = tf.reverse(w,[0])
        if h != None:
            v = sigmoid(dense(h,h.shape[1],k_h*k_w*k*input_shape[3],name='v'))
            v = tf.reduce_sum(v,0)
            v = tf.reshape(v, [k_h, k_w, k, input_shape[3]])
            w = w*v
...
def causal_deconv2_p(x, k, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.2, output_size=None,
             name='deconv2d', with_w=False, is_print=True):
    with tf.variable_scope(name):
        input_shape = x.get_shape().as_list()
        k = int(k/2)
        ...
        if k_h > 1:
            deconv1 = deconv1[:,kh:-kh,kw-1:-kw,:]
        else:
            deconv1 = deconv1[:,:,kw-1:-kw,:]
        x_roll = tf.manip.roll(x, shift=1, axis=1)
        deconv2 = tf.nn.conv2d_transpose(x_roll, w, output_shape=output_shape, strides=[1, d_h, d_w, 1],
                                        padding='VALID')
        
        if k_h > 1:
            deconv2 = deconv2[:,kh:-kh,kw-1:-kw,:]
        else:
            deconv2 = deconv2[:,:,kw-1:-kw,:]
        deconv = tf.concat([deconv1, deconv2], axis=3)
        biases = tf.get_variable('biases', [output_shape[-1]*2], initializer=tf.constant_initializer(0.0))
        deconv = tf.nn.bias_add(deconv, biases)
        if is_print:
        ...
        if with_w:
            return tf.reverse(deconv,[1]), w
        else:
            return tf.reverse(deconv,[1])
